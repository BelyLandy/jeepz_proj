...
public class VerletCloth2D : MonoBehaviour
{
    ...
    private void BuildCloth()
    {
        ...
        RebuildTriangles(); // первичная генерация треугольников
    ...
    {
        // колесико — размер курсора (как в статье) :contentReference[oaicite:1]{index=1}
        ...
        // ЛКМ удерживаем — тянем точки (через prevPos как в статье) :contentReference[oaicite:2]{index=2}
    ...
    {
        // clamp как в статье: ограничиваем компонентно (elasticity) :contentReference[oaicite:3]{index=3}
        ...
        for (int i = 0; i < _p.Length; i++)
        {
            ...
            {
                // "подкладываем" скорость через prevPos
                p.prevPos = p.pos - d;
                _p[i] = p;
            }
        }
    ...
    private void CutAt(Vector2 mouseWorld)
    {
        ...
        // 1) (опционально) удаляем точки — это дает РЕАЛЬНУЮ дырку в меше
        ...
        // 2) рвём constraints вокруг курсора (как tearing в статье) :contentReference[oaicite:4]{index=4}
        if (breakConstraintsOnCut)
        {
            for (int i = 0; i < _c.Count; i++)
            {
                ...
                // если точка удалена — связь тоже выключаем
                ...
                // “попали курсором” в любую из двух точек — рвём связь
    ...
    private void Simulate(float dt)
    {
        ...
        // Verlet integration: newPos = pos + (pos-prevPos)*(1-drag) + acc*(1-drag)*dt^2 :contentReference[oaicite:5]{index=5}
        for (int i = 0; i < _p.Length; i++)
        {
            var p = _p[i];
        ...
        // удовлетворяем constraints несколько раз
    ...
    private void SatisfyConstraints()
    {
        for (int i = 0; i < _c.Count; i++)
        {
            ...
            // как в статье: diffFactor = (rest - dist) / dist; offset = diff * diffFactor * 0.5 :contentReference[oaicite:6]{index=6}
    ...
    private void UpdateMesh()
    {
        ...
        for (int i = 0; i < _p.Length; i++)
        {
            // удаленные вершины в треугольниках не участвуют, позиция не важна
    ...
    // === Public API ===
    // Вырезать дырку кругом в world-точке
