...
public class Cloth2D : MonoBehaviour
{
    ...
    // демпфирование/масса: сделаем ткань потяжелее и глуше
    ...
    // ===== ПЛАЩ: крепление к герою =====
    [Header("Cape attach")]
    public Rigidbody2D attachTarget;            // Rigidbody2D героя
    public bool attachOnStart = true;           // авто-крепление при старте
    ...
    public bool attachAsSpring = true;          // тип джойнтов к герою
    [Min(1)] public int attachStride = 2;       // каждый N-й узел верхнего края
    ...
    {
        _pinsActive = false; // перестроим — отключим пины
        CleanupNodes();
        BuildMesh();
        BuildNodes();
        ApplyPins();
    ...
    void FixedUpdate()
    {
        // обновляем позиции "прибитых" к плечам узлов (кинематические пины)
    ...
    void BuildNodes()
    {
        ...
        for (int x = 0; x < width; x++)
        {
            ...
            // демпфирование с вертикальным градиентом (ниже — больше)
    ...
    // один метод для всех внутренних связей с усилением частоты/демпфирования
    void Link(Rigidbody2D a, Rigidbody2D b, float springMul)
    {
        ...
        else
        {
            ...
            j.autoConfigureDistance = true;
            j.enableCollision = enableCollisionOnJoints;
            j.maxDistanceOnly = useMaxDistanceOnly; // держим растяжение, сжатие свободнее
        }
    }
    ...
    // ====== Крепление к герою через ДЖОЙНТЫ (если нужно) ======
    ...
    void AddHeroJoint(GameObject node, Rigidbody2D target, bool asSpring, bool useAnchors, Vector2 localAnchor)
    {
        ...
        if (asSpring)
        {
            ...
            j.distance = 0f; // без стартового провиса
    ...
    // ====== Крепление к герою через КИНЕМАТИЧЕСКИЕ ПИНЫ (рекомендуется) ======
    ...
    public void PinTopToHeroShoulders(Rigidbody2D target, Vector2 leftLocal, Vector2 rightLocal)
    {
        ...
        // мгновенно выставим верхний ряд в нужное место (с учётом offset)
        for (int x = 0; x < width; x++)
        {
            float t = (width <= 1) ? 0f : (float)x / (width - 1);
            Vector2 local = Vector2.Lerp(_pinLeftLocal, _pinRightLocal, t) + capeOffsetLocal;
            ...
            // уберём любые Joint2D, если остались
    ...
    void UnpinTop()
    {
        ...
        for (int x = 0; x < width; x++)
        {
            ...
            if (!_pinsActive && rb.bodyType != RigidbodyType2D.Dynamic)
                rb.bodyType = RigidbodyType2D.Dynamic; // вернём динамику, если не используем пины
        }
    }
