...
public sealed class PlasmaAmmoShard25D : MonoBehaviour
{
    ...
    private CharacterPlasmaGlove25D plasmaGlove;
    ...
    public void Initialize(CharacterPlasmaGlove25D targetPlasmaGlove, Transform targetReceivePoint, int ammoToGrant, Vector3 initialScatterDirection)
    {
        plasmaGlove = targetPlasmaGlove;
        receiveTarget = targetReceivePoint;
        ammoAmount = Mathf.Max(0, ammoToGrant);
    ...
    private void Update()
    {
        ...
        if (plasmaGlove == null || receiveTarget == null)
        {
            Destroy(gameObject);
            return;
        }
    ...
    private void EnterArrivalFadeState()
    {
        ...
        if (!ammoGranted && plasmaGlove != null && ammoAmount > 0)
        {
            plasmaGlove.AddAmmo(ammoAmount);
            ammoGranted = true;
        }
    ...
    private Vector3 GetCurrentTargetPosition()
    {
        ...
        if (plasmaGlove != null)
            return plasmaGlove.transform.position;
