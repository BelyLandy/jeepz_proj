...
/// Применяет позы как в клипе Idle_Greeting, но проигрывает по фазе бита.
public class IdleGreetingFromCode : MonoBehaviour
{
    ...
    public bool Additive = false;          // добавлять к исходной позе
    public float Intensity = 1f;           // масштаб амплитуды

    // Сохраним базовые позы для Additive
    Vector3 _headBase, _bodyBase, _shoulderBase;

    // Нормализованные кривые (t 0..1) — с тангентами 0, как в YAML
    ...
    {
        // За сколько битов проигрываем полный цикл кривых
        float tNorm = Mathf.Repeat(beatPhase * BeatsPerLoop, 1f); // 0..1
        ...
        if (Intensity != 1f)
        {
            // Масштабируем отклонение от «средней» позы (середина кривой ~ ключ в 0 и 1)
